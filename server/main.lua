RegisterServerEvent('qb-overlord-laundering:use', function(key)
	...
	if Player.PlayerData.money.cash >= tonumber(machine.cost) then
		...
		TriggerClientEvent('QBCore:Notify', source, "Baza só!!", "error")
	end
end)
...
RegisterServerEvent('qb-overlord-laundering:load', function(machine)
	...
	if num_bags < 1 then
		TriggerClientEvent('QBCore:Notify', src, 'E o Dinheiro? Tambem és burro', 'error')
	...
	TriggerClientEvent('QBCore:Notify', src, 'Tu carregas-te '..num_bags..'x '..QBCore.Shared.Items['markedbills'].label..' ($'..total_worth..')', 'success')
