	...
	while true do Citizen.Wait(0)
		...
		for k,v in pairs(CONFIG['Machines']) do
			if k == currentMachine and not v.available and #(v.vec - globalCoords) < 5.0 then
				if v.finished then
					DrawText3Ds(v.vec.x, v.vec.y, v.vec.z, '~g~E~w~ - Colectar')
					...
					DrawText3Ds(v.vec.x, v.vec.y, v.vec.z, '~r~A Lavar o dinheiro sujo')
					notWashing = false
				end
			else
				if not displayed and #(v.vec - globalCoords) < 1.0 then
					if v.available and notWashing then
						DrawText3Ds(v.vec.x, v.vec.y, v.vec.z, '~g~E~w~ - Lavar dinheiro ($'..v.cost..')')
						if IsControlJustPressed(0,38) then
							TriggerServerEvent('qb-overlord-laundering:load', k)
							currentMachine = k
						end
					else
						DrawText3Ds(v.vec.x, v.vec.y, v.vec.z, '~r~Maquina Ocupada')
					end
